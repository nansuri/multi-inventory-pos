...
import { Package, ArrowRight, ShieldCheck, Lock, Sun, Moon } from 'lucide-vue-next';
...
<template>
  <div class="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 p-6 font-sans transition-colors duration-300">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-[3rem] shadow-2xl shadow-slate-200 dark:shadow-none w-full max-w-md overflow-hidden border border-slate-100 dark:border-slate-800 animate-in zoom-in-95 duration-500">
      <div class="p-10 pt-12">
        <div class="flex flex-col items-center mb-10 text-center">
          <div class="bg-indigo-600 p-4 rounded-3xl text-white shadow-xl shadow-indigo-200 dark:shadow-none mb-6">
        ...
        <div v-if="authStore.error" class="bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 p-4 rounded-2xl mb-8 text-sm font-bold border border-red-100 dark:border-red-900/50 flex items-center gap-2">
        ...
        <form @submit.prevent="login" class="space-y-6">
          ...
          <div class="space-y-2">
            ...
            <div class="relative">
              <input 
                ...
                class="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-transparent rounded-2xl font-bold focus:bg-white dark:focus:bg-slate-900 focus:border-indigo-600 transition-all outline-none dark:text-slate-100"
              />
              <Lock class="absolute right-6 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300 dark:text-slate-600" />
            </div>
          </div>
          ...
          <button 
            ...
            class="w-full bg-indigo-600 text-white py-5 rounded-[1.5rem] font-black shadow-xl shadow-indigo-100 dark:shadow-none hover:bg-indigo-700 disabled:opacity-50 transition-all flex items-center justify-center gap-3 group"
          >
        ...
        <div class="mt-12 text-center">
          <router-link 
            to="/register" 
            class="inline-block px-8 py-3 bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 text-indigo-600 dark:text-indigo-400 rounded-2xl font-black hover:border-indigo-600 dark:hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-950/20 transition-all"
          >
