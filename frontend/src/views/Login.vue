    ...
    <div class="bg-white rounded-[3rem] shadow-2xl shadow-slate-200 w-full max-w-md overflow-hidden border border-slate-100 animate-in zoom-in-95 duration-500">
      <div class="p-10 pt-12">
        ...
        <form @submit.prevent="login" class="space-y-6">
          <div class="space-y-2">
            <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">{{ $t('common.name') }} (Username)</label>
            <input 
              v-model="username" 
              type="text" 
              required
          ...
          >
            {{ authStore.loading ? $t('common.loading') : 'Login' }}
